...
export const WebcamPixelGrid: React.FC<WebcamPixelGridProps> = ({
  ...
  // Helper to parse any color string (hex, rgb, rgba)
  ...
  const monoRGB = React.useMemo(() => parseColor(monochromeColor), [monochromeColor]);

  // Parse border color
  const borderRGB = React.useMemo(() => parseColor(borderColor), [borderColor]);
